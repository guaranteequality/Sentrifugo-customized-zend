...
class Default_Form_Organisationhead extends Zend_Form
{
	public function init()
	{
        ...
        //for emp id
        $identityModel = new Default_Model_Identitycodes();
        $employeeinfo = $identityModel->fetchRow();
        if (!$employeeinfo['employeeid_length']) {
        	$employeeid_length = 6;
        }
        else
        	$employeeid_length = $employeeinfo['employeeid_length'];
        $employeeNumId = new Zend_Form_Element_Text("employeeNumId");
        $employeeNumId->setRequired("true");
        $employeeNumId->setLabel("Employee Id");
        $employeeNumId->setAttrib('maxLength', $employeeid_length);
