...
class Default_Form_wizardpreference extends Zend_Form
{
	public function init()
	{
		...
		$empidLength = new Zend_Form_Element_Text('employeeid_length');
        $empidLength->addFilter(new Zend_Filter_StringTrim());
		$empidLength->setAttrib('maxLength', 2);
		$empidLength->setRequired(true);
        $empidLength->addValidator('NotEmpty', false, array('messages' => 'Please enter employee code.')); 
		$empidLength->addValidators(array(array('StringLength',false,
								array('min' => 1,
									  'max' => 5,
									  'messages' => array(
									   Zend_Validate_StringLength::TOO_LONG =>
									  ...
									  Zend_Validate_StringLength::TOO_SHORT =>
									  'Employee code must contain at least %min% characters.')))));
		$empidLength->addValidators(array(
			         array(
			             'validator'   => 'Regex',
			             'breakChainOnFailure' => true,
			             'options'     => array( 
			             //'pattern' => '/^[A-Za-z]+$/',
			         	 'pattern'=> '/^[0-9]*$/',
			                 'messages' => array(
			                     Zend_Validate_Regex::NOT_MATCH =>'Please enter valid employee id length.'
			                 )
			             )
			         )
			     ));
   ...
   $this->addElements(array($timezoneid,$id,$currencyid,$organisationid,$empcodeid,$dateformatid,$timeformatid,	  $othercurrencycode,  $othercurrencyname,
	    	$currencyname,$passwordid,$perm_country,$perm_state,$perm_city,$workcodename,$empCode,	$empidLength, $submit));
